...
fn main() {
    ...
    let app_ciphered = cipher.chars().zip(app.chars()).map(|(c, a)| {
        let c = c as u8;
        let a = a as u8;
        let c = c ^ a;
        c as char
    ...
    println!("Your password for {} is: {} [ {} + {} + {} ]", app, pass, prefix, secret, app_ciphered);
}